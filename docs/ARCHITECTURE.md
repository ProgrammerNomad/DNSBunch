...
- **Email T2 (Phase 1):** `dmarc_checker`, `spf_checker`, `dkim_checker`, `mx_lookup` under `/tools/*`; DMARC/SPF/MX use `run_all_checks` slices; DKIM uses `lookup_dkim_selector` for a user-provided selector.
